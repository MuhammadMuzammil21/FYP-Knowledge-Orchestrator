...
import { useUploadMeeting, useMeetingStatus } from '../../../src/lib/hooks/useMeetings';
...
export function UploadForm({ onUploadSuccess }: UploadFormProps) {
    ...
    // Effect to handle completion - MUST be in useEffect to avoid infinite loop
    useEffect(() => {
        if (processingStatus?.status === 'complete' && uploadedMeetingId) {
            // Notify parent
            if (onUploadSuccess) {
                onUploadSuccess(uploadedMeetingId);
            }
    ...
    }, [processingStatus?.status, uploadedMeetingId, onUploadSuccess]);
