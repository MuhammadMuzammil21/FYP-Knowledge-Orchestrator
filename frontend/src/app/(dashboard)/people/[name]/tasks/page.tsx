      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          ...
          {pendingTasks.length > 0 && (
            <div>
              <h2 className="mb-4 text-xl font-semibold">Pending ({pendingTasks.length})</h2>
                ...
                {pendingTasks.map((task) => (
                  <Card key={task.id} className="p-4">
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                      ...
                      <Badge variant="outline" className="w-fit shrink-0 sm:mt-0 mt-1">
                        {task.status}
                      </Badge>
                    </div>
                  </Card>
          ...
          {completedTasks.length > 0 && (
            <div>
              <h2 className="mb-4 text-xl font-semibold">Completed ({completedTasks.length})</h2>
              <div className="space-y-3">
                {completedTasks.map((task) => (
                  <Card
                    key={task.id}
                    className="p-4 opacity-75 md:hover:opacity-100 transition-opacity"
                  >
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                      <div className="flex items-start gap-3 flex-1 min-w-0">
                        <CheckCircle2 className="mt-1 h-5 w-5 text-accent shrink-0" />
                        <div className="flex-1 min-w-0">
                          <p className="font-medium line-through break-words leading-relaxed">
                            {task.description}
                          </p>
                          <div className="mt-2 flex flex-wrap items-center gap-2 sm:gap-4 text-sm text-muted-foreground">
                            <Link
        ...
        <div className="flex h-96 items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-muted-foreground/70">No tasks assigned to {decodedName} yet</p>
          </div>
        </div>
      )}
    </div>
