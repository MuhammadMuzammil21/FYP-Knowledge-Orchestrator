...
export const adaptRAGResponse = (apiResponse: any): RAGResponse => {
    return {
        ...
        context: apiResponse.context?.map((ctx: any) => ({
            chunk: ctx.chunk,
            score: ctx.score,
        })) ?? [],
    };
...
export const adaptConflicts = (apiResponse: any): Conflict[] => {
    return apiResponse.conflicts?.map((conflict: any) => ({
        type: conflict.type,
 ...
 */
export const adaptKnowledgeGraph = (apiResponse: any): import('@/types/domain.types').KnowledgeGraph => {
    return {
...
export const adaptPersonTasks = (apiResponse: any): import('@/types/domain.types').PersonTasks => {
    return {
        ...
        tasks: apiResponse.tasks?.map((task: any) => ({
            id: task.id,
