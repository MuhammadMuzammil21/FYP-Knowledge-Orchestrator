...
export async function createProject(data: CreateProjectRequest): Promise<Project> {
  const response = await apiClient.post<Project>(API_ENDPOINTS.PROJECTS_LIST, data);
...
export async function getProjects(params?: {
  ...
  const response = await apiClient.get<ProjectListResponse>(API_ENDPOINTS.PROJECTS_LIST, {
    params,
  });
  return response.data;
}
...
export async function getProject(projectId: string): Promise<ProjectDetail> {
  const response = await apiClient.get<ProjectDetail>(API_ENDPOINTS.PROJECT_DETAIL(projectId));
  return response.data;
}
...
export async function updateProject(
  ...
  const response = await apiClient.put<Project>(API_ENDPOINTS.PROJECT_UPDATE(projectId), data);
  return response.data;
}
