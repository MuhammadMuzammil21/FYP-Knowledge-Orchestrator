...
export async function registerVoiceIdentity(audioData: Blob): Promise<VoiceIdentityResponse> {
    ...
    const response = await apiClient.post<VoiceIdentityResponse>(
        API_ENDPOINTS.USER_VOICE,
        formData,
        {
...
export async function getNotifications(unreadOnly = false): Promise<{ notifications: NotificationResponse[] }> {
...
export async function getUnreadNotificationsCount(): Promise<{ count: number }> {
    const response = await apiClient.get<{ count: number }>(API_ENDPOINTS.USER_NOTIFICATIONS_UNREAD_COUNT);
    return response.data;
}

export async function markNotificationAsRead(id: number): Promise<NotificationResponse> {
    const response = await apiClient.put<NotificationResponse>(API_ENDPOINTS.USER_NOTIFICATION_READ(id));
    return response.data;
}
...
export async function getNotificationPreferences(): Promise<NotificationPreferences> {
    const response = await apiClient.get<NotificationPreferences>(API_ENDPOINTS.USER_NOTIFICATION_PREFERENCES);
    return response.data;
}
