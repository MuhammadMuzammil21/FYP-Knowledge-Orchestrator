...
export async function googleLogin(credential: string): Promise<TokenResponse> {
  const response = await apiClient.post<TokenResponse>(API_ENDPOINTS.AUTH_GOOGLE_LOGIN, {
    credential,
  });
  return response.data;
}
...
export async function signup(credentials: SignupCredentials): Promise<TokenResponse> {
  const response = await apiClient.post<TokenResponse>(API_ENDPOINTS.AUTH_SIGNUP, credentials);
  return response.data;
}
...
export async function updateUserProfile(data: { name?: string }): Promise<User> {
