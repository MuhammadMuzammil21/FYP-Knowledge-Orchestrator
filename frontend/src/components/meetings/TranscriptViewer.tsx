...
export function TranscriptViewer({
  ...
  const handleSegmentTextChange = (index: number, newText: string) => {
    setEditableSegments((prev) =>
      prev.map((seg, i) => (i === index ? { ...seg, _editText: newText, _modified: true } : seg))
    );
  };
  ...
  const handleSaveDraft = () => {
    const content = editableSegments.map((s) => `${s._editSpeaker}: ${s._editText}`).join('\n');
                ...
                >
                  ...
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    {seg._modified && (
                      <Badge
                        variant="outline"
                        className="text-[9px] h-4 px-1.5 bg-amber-500/10 text-amber-600 border-amber-500/20 ml-auto"
                      >
                        edited
                      </Badge>
                    )}
      ...
      <Dialog open={isHistoryOpen} onOpenChange={setIsHistoryOpen}>
        <DialogContent className="max-w-md max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Previous saved versions of this transcript</DialogDescription>
          </DialogHeader>
          <div className="mt-2">
            {historyLoading ? (
