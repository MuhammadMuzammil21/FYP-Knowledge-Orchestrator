...
export function GraphDetailPanel({ node, onClose }: GraphDetailPanelProps) {
  ...
  const cfg =
    NODE_TYPE_CONFIG[node.nodeType as keyof typeof NODE_TYPE_CONFIG] ?? NODE_TYPE_CONFIG.default;
  const darkMode =
  ...
  const connectionsByType = node.connections.reduce<Record<string, typeof node.connections>>(
    (acc, conn) => {
